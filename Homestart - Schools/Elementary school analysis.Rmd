...
avg.cohorts = rbind( pi.avg.cohorts, mainland.avg.cohorts) %>% 
 mutate( color='Average, 2006-2014',x=1:n(), xmin=x-delta, xmax=x+delta) # For horiz error bars 

.by.grade %>% 
  mutate( max.school.yr = max(School.Year)) %>% 
  filter( School.Year == max(School.Year)) %>% 
  select( -Total, -max.school.yr) %>% 
  melt( id.vars='School.Year', value.name='n', variable.name='Grade') %>% 
  ggplot( aes(Grade, n)) + geom_bar(stat='identity', fill='red') + 
  geom_errorbarh( data=pi.avg.cohorts, mapping=aes(x=x, xmin=xmin, xmax=xmax, y=avg, color=color), size=1 ) +
  scale_color_manual( values='black') + guides(color=guide_legend(title=NULL))
  geom_point( data=pi.avg.cohorts, mapping=aes(Grade,avg))


x = pi.avg.cohorts %>% ggplot(aes(Grade, avg)) + geom_point() + expand_limits(y=0) %>% 
  ggtitle('Peaks )
x
y = mainland.avg.cohorts %>% ggplot(aes(Grade, avg)) + geom_point() + expand_limits(y=0)
y
ggplot_build(x)
x %>% ggplot_build
